...
func TestCountSecondValidPassphrases(t *testing.T) {
	tests := []struct {
		name string
		ps []string
		want int
	}{
		{
			...
			name: "a valid passphrase",
			ps: []string{
				"hello",
			},
			want: 1,
		},{
			name: "an invalid passphrase due to duplicated word",
			ps: []string{
				"hello hello",
			},
			want: 0,
		},{
			name: "an invalid passphrase due to duplicated anagrams",
			ps: []string{
				"hello lohel",
			},
			want: 0,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := CountSecondValidPassphrases(tt.ps); got != tt.want {
				t.Errorf("CountSecondValidPassphrases() = %v, want %v", got, tt.want)
			}
		})
	}
}