...
func TestSumLines(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "one line with a multiplication",
			args: args{
				rawInput: "3 * 3",
			},
			want: 9,
		...
		{
			name: "two lines, one with sum and another with multiplication",
			args: args{
				rawInput: "3 * 3\n1 + 2",
			},
			want: 12,
		...
		{
			name: "one line with sum and parentheses",
			args: args{
				rawInput: "1 + (2 + 3)",
			},
			want: 6,
		...
		{
			name: "one line with multiplication and parentheses",
			args: args{
				rawInput: "4 * (2 * 3)",
			},
			want: 24,
		...
		{
			name: "one line with sum, multiplication and parentheses",
			args: args{
				rawInput: "1 + (2 * 3)",
			},
			want: 7,
		...
		},
		{
			name: "fifth example",
			args: args{
				rawInput: "((2 + 4 * 9) * (6 + 9 * 8 + 6) + 6) + 2 + 4 * 2",
			},
			want: 13632,
		},
		{
			name: "given input",
			args: args{
				rawInput: `(9 + (5 + 2 + 2 * 4) * (7 + 7 * 5 * 3) + 7) + 2 + 4 * 2 + 3 * (8 + 5)
3 + 5 + (9 + 2 * 5) + (6 * 4 + 8) * 9 + 3
...
7 * 2 * 4 + 9`,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := SumLines(tt.args.rawInput); got != tt.want {
