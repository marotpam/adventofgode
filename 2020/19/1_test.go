...
func TestCountMessagesMatchingRules(t *testing.T) {
	type args struct {
		rawInput string
	}
	tests := []struct {
...
aaababbbabaababbabbbaabbbababaaaaaabbabbbbabbabaabababbbbbbababbabbbbaabbabababb`,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := CountMessagesMatchingRules(tt.args.rawInput); got != tt.want {
				t.Errorf("CountMessagesMatchingRules() = %v, want %v", got, tt.want)
			}
		})
	}
}
...
func TestGenerateAll(t *testing.T) {
	t.Run("only one rule with an exact match", func(t *testing.T) {
		r := rules(map[int]rule{
			0: {
				exactMatch: "a",
			},
		})

		assertPresent(t, r.generateAll(), "a")
	})
	t.Run("a rule with an exact match multiple times", func(t *testing.T) {
		r := rules(map[int]rule{
			0: {
				innerRules: [][]int{
					{1, 1, 1},
		...
		all := r.generateAll()
		assertPresent(t, all, "aaa")
	})
	t.Run("a rule with multiple exact matches", func(t *testing.T) {
		r := rules(map[int]rule{
			0: {
				innerRules: [][]int{
					{1, 2, 1},
		...
		all := r.generateAll()
		assertPresent(t, all, "aba")
	})
}

func assertPresent(t *testing.T, all map[string]bool, messages ...string) {
	for _, m := range messages {
		_, ok := all[m]
		if !ok {
			t.Errorf("Expecting %q to be in all, but not found", m)
		}
	}
}
