...
func Test_parse(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "a bag containing two bags",
			args: args{
				lines: []string{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := parse(tt.args.lines); !reflect.DeepEqual(got, tt.want) {
...
func TestCountBagsContainingShinyGoldBag(t *testing.T) {
	type args struct {
		rawInput string
	}
	tests := []struct {
...
dotted black bags contain no other bags.`,
		...
		},
		{
			name: "given input",
			args: args{
				rawInput: `faded yellow bags contain 4 mirrored fuchsia bags, 4 dotted indigo bags, 3 faded orange bags, 5 plaid crimson bags.
...
bright bronze bags contain 1 clear magenta bag, 2 clear gray bags, 2 dull coral bags, 5 vibrant gray bags.`,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := CountBagsContainingShinyGoldBag(tt.args.rawInput); got != tt.want {
				t.Errorf("CountBagsContainingShinyGoldBag() = %v, want %v", got, tt.want)
			}
		})
	}
}