...
func TestParseGames(t *testing.T) {
	tcs := []struct {
		...
		{rawInput: "", expected: []game{}},
		{rawInput: "Game 1: 3 blue, 4 red", expected: []game{
			{
				{
					{colour: "blue", count: 3},
					{colour: "red", count: 4},
		...
		{rawInput: `Game 1: 3 blue, 4 red
		Game 2: 1 blue, 2 green; 3 green, 4 blue, 1 red; 1 green, 1 blue`, expected: []game{
			{
				{
					{colour: "blue", count: 3},
					{colour: "red", count: 4},
	...
	for _, tc := range tcs {
		if got := parseGames(tc.rawInput); !reflect.DeepEqual(got, tc.expected) {
			t.Errorf("expecting %+v\n got %+v\n for %+v", tc.expected, got, tc.rawInput)
		}
	}
}
...
Game 5: 6 red, 1 blue, 3 green; 2 blue, 1 red, 2 green
		...
		// input part 1
		{
			rawInput: `Game 1: 1 red, 5 blue, 1 green; 16 blue, 3 red; 6 blue, 5 red; 4 red, 7 blue, 1 green
	...
	for _, tc := range tcs {
		if got := SumIdsOfPossibleGames(tc.rawInput); !reflect.DeepEqual(got, tc.expected) {
			t.Errorf("expecting %+v got %+v for %+v", tc.expected, got, tc.rawInput)
		}
	}
}
