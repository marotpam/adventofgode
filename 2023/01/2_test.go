...
func TestStringsToNumber(t *testing.T) {
	result := StringsToNumbers("onetwothreefourfivesixseveneightnine")
	if result != "123456789" {
		t.Errorf("expected %s, actual %s", "123456789", result)
	}

	result2 := StringsToNumbers("oneighthreetwoone")
	if result2 != "18321" {
		t.Errorf("expected %s, actual %s", "18321", result2)
	}
}

func TestSecondPartExample(t *testing.T) {
	n := SumCalibrationValuesPart2(`
	two1nine
	eightwothree
	abcone2threexyz
	xtwone3four
...
`)
	if n != 281 {
		t.Errorf("expected %d, actual %d", 281, n)
	}
}
...
`)
	expected := 53389
	if n != 53389 {
		t.Errorf("expected %d, actual %d", expected, n)
	}
}
