...
func TestGetCalibrationValue(t *testing.T) {
	n := GetCalibrationValue("1abc2")
	if n != 12 {
		t.Errorf("expected %d, actual %d", 12, n)
	}
}

func TestSumCalibrationValuesPart1(t *testing.T) {
	n := SumCalibrationValuesPart1(`
1abc2
pqr3stu8vwx
a1b2c3d4e5f
treb7uchet
`)
	if n != 142 {
		t.Errorf("expected %d, actual %d", 142, n)
	}
}
...
`)
	if n != 54338 {
		t.Errorf("expected %d, actual %d", 54338, n)
	}
}
