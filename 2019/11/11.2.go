...
func (r *robot) print(instructions []int) {
	...
	for panel := range r.paintedPanels {
		if panel.x < min.x {
			min.x = panel.x
		}
		if panel.x > max.x {
	...
	for y := max.y; y >= min.y; y-- {
		for x := min.x; x <= max.x; x++ {
			c, ok := r.paintedPanels[position{x: x, y: y}]
			if ok && c == colourWhite {
				fmt.Print("#")
			} else {
				fmt.Print(" ")
...
func main() {
	...
	robot := newRobot()
	robot.paintCurrentPanel(colourWhite)

	robot.print(instructions) // HKJBAHCR
}
