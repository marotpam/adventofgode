...
func TestRelativeBase(t *testing.T) {
	t.Run("first example", func(t *testing.T) {
		...
		ints := []int{109, 1, 204, -1, 1001, 100, 1, 100, 1008, 100, 16, 101, 1006, 101, 0, 99}
		interpreter := NewInterpreter()
		interpreter.Run( ints, in, out)

		if !reflect.DeepEqual(ints, out.outputs) {
			t.Errorf("Failed asserting\n %#v matches \n %#v", out.outputs, ints)
		}
	})
	t.Run("second example", func(t *testing.T) {
		...
		ints := []int{1102, 34915192, 34915192, 7, 4, 7, 99, 0}
		interpreter := NewInterpreter()
		interpreter.Run( ints, in, out)

		if len(strconv.Itoa(out.outputs[0])) != 16 {
			t.Errorf("%d does not have %d digits, it has %d", out.outputs[0], 16, len(strconv.Itoa(out.outputs[0])))
		}
	})
	t.Run("third example", func(t *testing.T) {
		...
		ints := []int{104, 1125899906842624, 99}
		interpreter := NewInterpreter()
		interpreter.Run( ints, in, out)

		expectedOutput := []int{1125899906842624}
		if !reflect.DeepEqual(expectedOutput, out.outputs) {
			t.Errorf("Failed asserting\n %#v matches \n %#v", out.outputs, expectedOutput)
		}
	})
	t.Run("fourth example", func(t *testing.T) {
		in := &fakeInput{inputs: []int{10}}
		out := &fakeOutput{outputs: []int{}}

		ints := []int{203, 0, 99}
		interpreter := NewInterpreter()
		interpreter.relativeBase = 1000

		interpreter.Run(ints, in, out)

		if interpreter.memory[1000] != 10 {
			t.Error("not matched", interpreter.memory[1000])
		}
	})
}
